...
if (isset($_POST["store_name"]) && isset($_POST["store_address"]) && isset($_POST["new_password"]) && isset($_POST["new_password_confirm"])) {
    ...
    if (empty($store_name)) {
        header("Location: initializeAccount.php?state=請輸入店家名稱");
    } elseif (empty($store_address)) {
        header("Location: initializeAccount.php?state=請輸入店家地址");
    } elseif (empty($new_password)) {
        header("Location: initializeAccount.php?state=請輸入新密碼");
    } elseif (empty($new_password_confirm)) {
        header("Location: initializeAccount.php?state=請再次輸入新密碼");
    } else {
        if ($new_password == $new_password_confirm) {
            ...
            if ($result) {
                ...
                if ($result) {
                    ...
                    // 密碼未成功輸入至資料庫
                    header("Location: initializeAccount.php?state=發生問題，請再試一次");
                }
            } else {
                // 店名、地址未成功輸入至資料庫
                header("Location: initializeAccount.php?state=發生問題，請再試一次");
            ...
            header("Location: initializeAccount.php?state=新密碼輸入不一致");
        }
    }
} else {
    // 參數傳遞失敗
    header("Location: initializeAccount.php?state=儲存失敗");
}
