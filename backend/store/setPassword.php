...
if (isset($_POST["old_password"]) && isset($_POST["new_password"]) && isset($_POST["new_password_confirm"])) {
    ...
    if (empty($old_password)) {
        header("Location: ../../store/resetPassword.php?state=請輸入舊密碼");
    } elseif (empty($new_password)) {
        header("Location: ../../store/resetPassword.php?state=請輸入新密碼");
    } elseif (empty($new_password_confirm)) {
        header("Location: ../../store/resetPassword.php?state=請再次輸入新密碼");
    } else {
        ...
        if (password_verify($old_password, $check_password)) {
            if ($new_password == $new_password_confirm) {
                ...
                if ($result) {
                    header("Location: ../../store/resetPassword.php?state=true");
                } else {
                    header("Location: ../../store/resetPassword.php?state=密碼重設失敗");
                }
            } else {
                header("Location: ../../store/resetPassword.php?state=新密碼輸入不一致");
            }
        } else {
            header("Location: ../../store/resetPassword.php?state=舊密碼輸入錯誤");
        }
    }
} else {
    // 參數傳遞失敗
